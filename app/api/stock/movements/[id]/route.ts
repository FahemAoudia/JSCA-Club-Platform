...
export async function GET(_request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const row = await db.stockMovement.findUnique({ where: { id } });
...
export async function PATCH(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (guard) return guard;
  const { id } = await ctx.params;
  const patch = (await request.json().catch(() => null)) as Prisma.StockMovementUpdateInput | null;
  if (!patch) return NextResponse.json({ ok: false, error: "invalid_request" }, { status: 400 });
  const updated = await db.stockMovement.update({ where: { id }, data: patch });
